...
import PairedBadge from "../PairedBadge";
import { SelectDropdownWithGroup } from "../Customs/SelectDropdownWithGroup";
export const NotificationsCreateRequest = ({
  ...
  const valActivityTypes = [
    ...
    {
      ExtendedValue: "01TYPE",
      Id: "01TYPE",
      Value: "COLD-Cold Sample"
    },
    {
      ExtendedValue: "BCP",
      Id: "BCP",
  ...
  const getNotificationType = async () => {
    const response = valNotificationType
    console.log("inside getNotificationType ", response);
    if (response) {
      let notifList = response.filter(
        (e) => ["M1", "O1", "PM"].indexOf(e.Id) >= 0
      );
      setNotificationType(notifList);
    }
  };
  const getDefectLocations = async (search) => {
    const response = valActivityTypes
    console.log("inside getDefectLocations ", response);
    if (response) {
  ...
  const getNotificationTechStatuses = async () => {
    const response = valActivityTypes
    console.log("inside getNotificationTechStatuses ", response);
    if (response) {
  ...
  const getNotificationPriorityType = async () => {
    const response = valPriorityTypes
    console.log("inside getNotificationPriorityType ", response);
    if (response) {
  ...
  const getNotificationCauseCodes = async () => {
    const response = valActivityTypes
    console.log("inside getNotificationCauseCodes ", response);
    if (response) {
  ...
  const getActivityTypes = async () => {
    const response = valActivityTypes
    console.log("inside getActivityTypes ", response);
    if (response) {
      setActivityTypes(response);
    }
  };
  const getActivityCodes = async () => {
    const response = valActivitySubTypes
    console.log("inside getActivityCodes subTypes ", response);
    if (response) {
  ...
  const onSubmit = (val) => {
    ...
    console.log("printing val inside onSubmit : ", val);
